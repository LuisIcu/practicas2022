...
data = convolve(A,gauss_2D_kernel)
...
for i in range(len(data)):
    for j in range(len(data[i])):
        ...
        zData= np.append(zData,data[i][j])
...
def func2(data, A, x1, y1, B, x2, y2):
    x = data[1]
    y = data[0]
    return A*np.exp(-((x-x1)**2+(y-y1)**2)/(18)) + B*np.exp(-((x-x2)**2+(y-y2)**2)/(18))

if __name__ == "__main__":    
    initialParameters = [15, 21, 27, 15,25,25] # these are the same as scipy default values in this example

    # here a non-linear surface fit is made with scipy's curve_fit()
    fittedParameters, pcov = scipy.optimize.curve_fit(func2, [yData, xData], zData, p0 = initialParameters)
